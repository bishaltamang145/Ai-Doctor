...
export function TileLayout({
  ...
  const cameraTracks = useTracks([Track.Source.Camera, Track.Source.Unknown]);
  ...
  const agentVideoTrack = useMemo(() => {
    ...
    return (
      remoteCameraTracks.find((track) => {
        const identity = track.participant.identity.toLowerCase();
        const name = (track.participant.name ?? '').toLowerCase();
        return identity.includes('avatar') || name.includes('avatar') || name.includes('bey');
      }) ??
      // A Bey avatar is the only remote camera publisher in a normal one-user room.
      (remoteCameraTracks.length > 0 ? remoteCameraTracks[0] : undefined)
    );
              ...
              {!isAvatar && (
                ...
                <motion.div
                  ...
                  className={cn(
                    ...
                    avatarUrl
                      ? 'border-2 border-slate-200 bg-slate-100 ring-4 ring-slate-100 shadow-md'
              ...
              {isAvatar && (
                ...
                >
                  <VideoTrack
                    ...
                    className={cn('size-full object-cover', !chatOpen && !isPip && 'max-h-[75vh] rounded-2xl mx-auto')}
                  />
                </motion.div>
              )}
            </AnimatePresence>
